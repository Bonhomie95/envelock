...
Lives in security/ (not api/) because the channel layer builds these values at
subscription time (channels/mail/providers.py) and the api layer verifies them
on delivery — neither should import the other for it.
...
def _configured() -> bool:
    """An empty secret must mean "reject every push", never "accept an HMAC of
    the empty string" — the latter is computable offline by anyone."""
    return bool(_secret())
...
def verify_client_state(value: str | None) -> tuple[UUID, str] | None:
    ...
    if not value or not _configured():
        return None
    parts = value.split("|")
    if len(parts) != 3:
        return None
...
def verify_push_token(value: str | None) -> bool:
    return bool(value) and _configured() and hmac.compare_digest(str(value), push_token())
