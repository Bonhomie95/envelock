...
class Notification:
    ...
    #: The full reader-facing report, when we have the message facts to build
    #: one. Optional: SMS and push are length-constrained and use `title`, and
    #: an alert can be raised by a channel with no single message behind it.
...
def _smtp_tls_context():
    """CA-verified TLS for SMTP submission.

    aiosmtplib's default context uses the interpreter's system CA store, which a
    macOS Python install often does not have — every STARTTLS then dies with
    CERTIFICATE_VERIFY_FAILED before a byte of mail moves. certifi ships a real
    bundle and is already in the dependency tree (httpx); fall back to the
    system store where certifi is absent.
    """
    import ssl
...
class EmailSender(Sender):
    ...
    @property
    def configured(self) -> bool:
        # `localhost` is the `.env.example` default and almost never a real
        # relay. Treating it as configured makes the cascade spend a connection
        # attempt per Critical alert and report "connect call failed" instead of
        # the truth, which is that nobody set this up. Same rule as
        # `notify.mail.is_configured`, so the two paths cannot disagree.
        return bool(self.host and self.host != "localhost" and self.from_addr)
    ...
    def build(self, notification: Notification, *, to: str) -> EmailMessage:
        ...
        if notification.report is not None:
            # The structured report: which message, what we found, and whether
            # anything is required of the reader. See notify/report.py.
            from envelock.notify.report import render_html, render_text

            msg.set_content(render_text(notification.report))  # type: ignore[arg-type]
            msg.add_alternative(
                render_html(notification.report),  # type: ignore[arg-type]
                subtype="html",
            )
            return msg

        # No message behind this alert (or an older caller): the plain body.
