...
def _sender_domain(ctx: DetectionContext) -> str:
    """The sender's registrable domain, for detections that must work on a
    counterparty we have no row for yet (first contact)."""
    mail = ctx.mail
    return registrable_domain(mail.sender.domain) if mail is not None else ""
...
class _A2BankRegistry:
    ...
    def evaluate(self, ctx: DetectionContext) -> list[FindingResult]:
        cp = ctx.counterparty
        if not _external(ctx):
            return []
        if not has_payment_context(_body(ctx)) or not extract_bank_identifiers(_body(ctx)):
            return []
        if cp is not None and cp.verified_phone:
            # A verified callback number used to silence A2 entirely — which
            # left the DILIGENT tenant uncovered: a thread-hijacked known vendor
            # sending "our new IBAN" produced nothing when no bank record was on
            # file (A1 needs a baseline to diff against). First-ever bank
            # details from a known correspondent are exactly what the verified
            # number exists for.
            if cp.known_bank_ids:
                return []  # baseline exists — a mismatch is A1's job
            return [
                FindingResult(
                    service="A2",
                    ...
                    score=40,
                    summary=(
                        f"First bank details we've seen from "
                        f"{cp.registrable_domain}. Call the number you have on "
                        f"file for them before paying — never a number from "
                        f"this email."
                    ),
                    evidence={
                        "counterparty": cp.registrable_domain,
                        "first_bank_details": True,
                        "callback_phone": cp.verified_phone,
                    },
                )
            ]

        # A sender we have never heard from has no counterparty row yet —
        # `learn()` creates it only after detection, deliberately, so a
        # fraudulent message cannot poison the baseline it is judged against.
        #
        # A2 used to bail out on `cp is None`, which quietly made the entire
        # novel-vendor family in `risk/engine.py` unreachable. Those combinations
        # — {A2,A7,A14}, {A2,A3}, {A2,A4}, {A2,A5}, {A2,A6}, {A2,B7} — exist
        # specifically for "no prior A1 baseline to diff against", and every one
        # of them pairs A2 with a signal that only fires on FIRST contact. But
        # A7 returns nothing once `message_count > 0`, and A2 returned nothing
        # until a row existed. The two could never fire on the same message, so
        # the first-strike BEC path A1 structurally cannot catch was itself
        # never caught: those messages topped out at HIGH with no callback
        # prompt, instead of CRITICAL with "phone them before you pay".
        #
        # Semantically the no-row case is the *strongest* form of what A2
        # reports: there is certainly no verified callback number on file for a
        # payee we have never seen. So it fires, naming the sender's domain.
        domain = cp.registrable_domain if cp is not None else _sender_domain(ctx)
        if not domain:
            return []
        return [
            FindingResult(
                ...
                score=35,
                summary=(
                    f"No verified phone number on file for {domain}. "
                    f"Add one so payment changes can be checked out of band."
                ),
                evidence={
                    "counterparty": domain,
                    "has_bank_records": bool(cp.known_bank_ids) if cp else False,
                    "first_contact": cp is None,
                },
            )
        ]
...
# Invoice/amount extraction is shared with the learning side (util/payments) —
# what A13 compares against must be extracted the same way it was learned.
from envelock.util.payments import extract_amounts as _extract_amounts  # noqa: E402
from envelock.util.payments import extract_invoice_numbers as _extract_invoices  # noqa: E402
...
class _B2TimeOfClick:
    ...
    def evaluate(self, ctx: DetectionContext) -> list[FindingResult]:
        ...
        if mail is None or not _external(ctx) or not mail.urls:
            return []
        # The rewriting itself happens in the enforcement layer
        # (channels/mail/enforce + workers/imap_fetch); this finding records
        # that the mailbox qualifies and how many links are covered.
        return [
            FindingResult(
                service="B2",
                tier=AlertTier.LOW,
