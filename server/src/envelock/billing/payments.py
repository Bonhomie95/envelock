...
class PaymentProvider(Protocol):
    ...
    #: Whether `verify_instrument` on this provider constitutes real payment
    #: authorisation — i.e. proof strong enough to open the entitlement gate.
    #: A Stripe `GET /payment_methods/{pm}` proves a PaymentMethod OBJECT exists
    #: (mintable client-side with the public key and any Luhn-valid PAN); it
    #: proves nothing about the card being chargeable. Entitlement therefore
    #: comes ONLY from a verified paid event (the Checkout webhook) — or, in
    #: development, from the sandbox, which exists to exercise that flow.
...
class HostedCheckoutProvider(PaymentProvider, Protocol):
    """A provider that can host the whole payment page itself.

    Split out rather than folded into `PaymentProvider` because it is genuinely
    not universal: Stripe Checkout and its billing portal are hosted pages we
    redirect to, while the regional acquirers here are API-only. Declaring these
    on the base protocol would have every provider claim a capability most of
    them do not have — and `provider_for()` returns the base type, so the
    checkout routes were calling methods the type system had no reason to
    believe existed.
    """
...
def hosted_checkout_provider(name: str) -> HostedCheckoutProvider | None:
    """`provider_for`, narrowed to providers that can host a checkout page."""
    provider = _PROVIDERS.get(name.lower())
    if provider is None or not hasattr(provider, "create_checkout_session"):
        return None
...
class _Sandbox:
    ...
    id: str = "sandbox"
    #: The sandbox exists to exercise the post-payment flow locally, so it is
    #: the one provider whose verification opens the gate — and it can never be
    #: configured outside development.
    grants_entitlement: bool = True
...
# Built as a list first: a dict comprehension over a heterogeneous tuple infers
# the union of the concrete classes as the value type, which is not assignable to
# the protocol even though every member satisfies it structurally.
_ALL_PROVIDERS: list[PaymentProvider] = [
    _Stripe(),
    _Adyen(),
    _MercadoPago(),
    _Razorpay(),
...
_PROVIDERS: dict[str, PaymentProvider] = {p.id: p for p in _ALL_PROVIDERS}
