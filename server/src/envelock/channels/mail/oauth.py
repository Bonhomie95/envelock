...
class _Microsoft:
    ...
    #: Graph mail read, with a refresh token for background sync. All three are
    #: USER-consentable — any employee can connect their own mailbox with no IT
    #: admin involved. AuditLog.Read.All (sign-in-log ingest for C3/C5/C13) was
    #: deliberately dropped: it requires org admin consent and only exists on
    #: work accounts, so it turned every Microsoft connect into an admin-consent
    #: wall. Those login detections are covered by the client sensor instead;
    #: the mailbox therefore does NOT claim the ENTRA_LOGS source (see
