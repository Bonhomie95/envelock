...
async def resolve_tenant_by_token(token: str) -> UUID | None:
    ...
    # The token is how we learn the tenant, so this lookup necessarily spans
    # tenants. Unscoped under row-level security it found nothing, and every
    # forwarded message was dropped as "unknown address".
    with system_scope("forward ingest: resolve the ingest token"):
...
async def _mailbox_for(session, tenant_id: UUID, raw: bytes) -> Mailbox | None:
    ...
    peek = await parse_message_async(
        raw,
        tenant_id=tenant_id,
...
async def run_forwarded_message(tenant_id: UUID, raw: bytes) -> dict:
    ...
    set_current_tenant(tenant_id)  # everything below is this tenant's data
    async with get_sessionmaker()() as session:
        # Same seat-cap enforcement as the pollers: a lapsed unpaid trial must
        # not keep live protection through its forwarding address either.
        ...
        if tenant is None or not mailbox_entitled(tenant):
            return {"alerted": False, "reason": "tenant not entitled (trial lapsed?)"}
