...
#: python-docx inflates the whole OOXML zip into lxml trees — a deflate bomb
...
def _docx_text(data: bytes) -> str:
    if len(data) > _MAX_DOCX_BYTES:
        logger.debug("docx too large to extract (%d bytes)", len(data))
        return ""
    try:
...
def extract(data: bytes, *, mime: str | None, filename: str) -> tuple[str, tuple[str, ...]]:
    ...
    if mime in _PDF_MIMES or name.endswith(".pdf"):
        return _pdf_text(data)[:_MAX_EXTRACT_CHARS], ()
    if mime in _DOCX_MIMES or name.endswith(".docx"):
        return _docx_text(data)[:_MAX_EXTRACT_CHARS], ()
    if mime.startswith(_IMAGE_PREFIX) or name.endswith(
        (".png", ".jpg", ".jpeg", ".gif", ".bmp", ".tiff", ".webp")
    ):
        return _image_ocr(data)[:_MAX_EXTRACT_CHARS], _image_qr_urls(data)
    return "", ()
