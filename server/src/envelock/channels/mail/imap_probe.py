...
def _connect(
    candidate: Candidate, timeout: float, *, pin_sha256: str | None = None
) -> imaplib.IMAP4:
    """Open one IMAP connection, honouring an approved-certificate pin.

    With no pin this is ordinary strict TLS: Python's default context verifies
    the chain and the hostname. With a pin we deliberately skip that and check
    the certificate's fingerprint instead — narrower, not weaker. Standard
    verification asks "is this a certificate some authority vouches for, for
    this name?"; a pin asks "is this the exact certificate the customer looked
    at and approved?", which no machine-in-the-middle can satisfy.

    The pin is checked before a single byte of credential is sent.
    """
    if candidate.security == "ssl":
        if pin_sha256:
            ctx = unverified_context()
            client: imaplib.IMAP4 = imaplib.IMAP4_SSL(
                candidate.host, candidate.port, ssl_context=ctx, timeout=timeout
            )
            _check_pin_or_close(client, pin_sha256)
            return client
        # An explicit context, NOT the default. `imaplib.IMAP4_SSL(host, port)`
        # builds an unverifying one, so the obvious call would send the mailbox
        # password to any server that answered. See imap_tls.verifying_context.
        return imaplib.IMAP4_SSL(
            candidate.host, candidate.port, ssl_context=verifying_context(), timeout=timeout
        )

    if candidate.security != "starttls":
        # Neither "ssl" nor "starttls": this connection would carry the mailbox
        # password in clear text. The STARTTLS branch below has always refused to
        # degrade for exactly this reason — the no-encryption mode simply slipped
        # past it, because it never reaches that branch at all.
        from envelock.config import get_settings

        if not get_settings().imap_allow_plaintext:
            raise OSError(
                "refusing an unencrypted IMAP connection — the mailbox password "
                "would cross the internet in clear text"
            )
...
def _check_pin_or_close(client: imaplib.IMAP4, pin_sha256: str) -> None:
    """Verify the pin, and hang up rather than talk to the wrong server."""
    try:
        verify_pin(client.sock, pin_sha256)  # type: ignore[arg-type]
    except CertificatePinError:
        # Close hard. `logout()` would send LOGOUT to a peer we have just
        # decided we do not trust; there is nothing to be polite about.
...
def _authenticate_password(client: imaplib.IMAP4, username: str, password: str):  # noqa: ANN202
    """Sign in with a password, trying what the server will actually accept.

    `imaplib.login()` sends the `LOGIN` command and nothing else. Plenty of
    servers — Dovecot with `disable_plaintext_auth`, and most corporate mail —
    advertise `LOGINDISABLED` and expect a SASL mechanism instead. Thunderbird
    speaks several, which is exactly why a mailbox connects there and not here.

    Order matters and is not arbitrary:

    * `LOGIN` first when it is allowed. It is what every server supports, and
      the credential is already protected by TLS.
    * `CRAM-MD5` next. MD5 is long dead for signatures, but as an HMAC
      challenge-response inside TLS it is what these servers ask for, and it is
      strictly better than not connecting: the password itself never crosses
      the wire even once.
    * `PLAIN` last, over the SASL path, for servers that disable `LOGIN` but
      accept the same credential a different way.

    Returns imaplib's `(typ, data)`.
    """
    ...
    if not login_disabled:
        try:
            ...
            # Fall through to the SASL mechanisms below: a server can refuse
            # LOGIN without having advertised LOGINDISABLED.
            pass

    if "AUTH=CRAM-MD5" in caps:
        with contextlib.suppress(imaplib.IMAP4.error):
            return client.login_cram_md5(username, password)

    if "AUTH=PLAIN" in caps:
        with contextlib.suppress(imaplib.IMAP4.error):
            return client.authenticate(
                "PLAIN", lambda _: f"\0{username}\0{password}".encode()
            )

    if login_disabled:
        # Nothing we speak is on offer. Say so precisely rather than reporting a
        # generic auth failure, because the fix is completely different.
        offered = sorted(c[5:] for c in caps if c.startswith("AUTH="))
        raise imaplib.IMAP4.error(
            "LOGINDISABLED: the server refuses password sign-in and offers only "
            f"{', '.join(offered) or 'no mechanisms we support'}"
        )
    # LOGIN was allowed and failed: re-run it so the real rejection surfaces.
    return client.login(username, password)
...
def try_login(
    ...
    *,
    ...
    `pin_sha256` is a certificate the customer has explicitly approved for this
    mailbox; with one set, the connection is checked against that exact
    certificate instead of the public CA chain.
    """
    ...
    try:
        client = _connect(candidate, timeout, pin_sha256=pin_sha256)
    ...
    try:
        if access_token is not None:
            ...
            typ, data = _authenticate_password(client, username, password or "")
        if typ != "OK":
            return Attempt(
                candidate,
                username,
