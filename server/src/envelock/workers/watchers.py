...
_RDAP_CLIENT: RdapClient | None = None


def get_rdap_client() -> RdapClient:
    """Process-wide RDAP client so its cache is shared by the mail pipeline,
    the scan endpoints and the watchers — per-request instances re-queried the
    registries for the same domains."""
    global _RDAP_CLIENT  # noqa: PLW0603
...
class RdapClient:
    ...
    #: The cache exists to spare registries repeat lookups, not to grow without
    #: bound — it is fed partly by unauthenticated scan input, so an attacker
    #: naming thousands of fresh domains must not translate into process memory.
    _CACHE_MAX = 4096
    ...
    async def lookup(self, domain: str) -> dict | None:
        ...
        # the oldest entry. Simple beats clever for a best-effort cache.
