...
async def persist_observation(session: AsyncSession, obs) -> int:  # noqa: ANN001
    ...
    for tenant_id, _domain_id in tenants:
        ...
        if has_mx:
            ...
            alerted += 1
            # Deliver now — an armed lookalike is time-sensitive, and relying on
            # the escalation sweep's side-effect delivery added up to a full
            # interval of silence (or forever, with the scheduler off).
