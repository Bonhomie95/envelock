...
class AttachmentRef(_Base):
    ...
    sha256: str
    size_bytes: int
    #: TRANSIENT — the raw bytes for this analysis pass only. Never persisted
    #: (Message stores hashes; events are not stored). Carried so the
    #: attachment cascade can triage content without a second fetch.
    raw: bytes | None = None
