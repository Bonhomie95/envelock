...
router = APIRouter(
    prefix="/api/v1/admin",
    tags=["admin"],
    # The operator console exists to see every tenant.
...
# ── System status: one honest health view of every moving part ───────────────
async def _timed_check(coro, *, timeout: float = 3.0) -> tuple[bool, str]:  # noqa: ASYNC109
    """Run one dependency check, bounded. The status page must never hang on the
    very thing that is down — its whole job is to stay up when others don't."""
    import asyncio

    try:
        async with asyncio.timeout(timeout):
            return await coro
    except Exception as exc:  # noqa: BLE001 — any failure is a red check, not a 500
...
async def _check_redis() -> tuple[bool, str]:
    ...
    if settings.rate_limit_backend != "redis":
        return True, "not configured (single-instance mode)"
...
async def system_status(actor: PlatformReader) -> dict:
    """Aggregated health of every part of Envelock, for the ops status page.

    Rolls up the API, datastores, the two background workers, outbound email,
    the AI cascade and the OAuth providers into one green/amber/red view. Each
    check is bounded so a down dependency can't hang the page. `overall` is the
    worst state present: `down` if anything critical failed, `degraded` if only
    non-critical checks are off, else `operational`.
    """
    from datetime import UTC, datetime

    from envelock.security.keys import custody_summary
    from envelock.workers.imap_fetch import worker_health
    from envelock.workers.scheduler import scheduler_health

    settings = get_settings()
    checks: list[dict] = []

    def add(component: str, ok: bool, *, critical: bool, detail: str) -> None:
        checks.append(
            {
                "component": component,
                "state": "up" if ok else ("down" if critical else "degraded"),
                "critical": critical,
                "detail": detail,
            }
        )
    ...
    # 1. Datastores.
    ok, detail = await _timed_check(_check_db())
    add("database", ok, critical=True, detail=detail)
    ok, detail = await _timed_check(_check_redis())
    add("redis", ok, critical=False, detail=detail)

    # 2. IMAP poll worker — up if it ran recently and isn't overrunning its
    #    cadence. Cycle duration approaching the interval is the scale-up signal.
    wh = worker_health()
    interval = settings.imap_poll_worker_seconds
    dur = float(wh.get("duration_seconds") or 0.0)
    if not settings.imap_poll_worker_enabled:
        add("imap_poll_worker", True, critical=False, detail="disabled in config")
    elif wh.get("ran_at") is None:
        add("imap_poll_worker", False, critical=False, detail="no cycle recorded yet")
    else:
        lagging = dur > interval * 0.8
        add(
            "imap_poll_worker",
            not lagging,
            critical=False,
            detail=(
                f"last cycle {dur:.1f}s over {wh.get('mailboxes', 0)} mailbox(es), "
                f"interval {interval}s"
                + (" — approaching interval, consider scaling up" if lagging else "")
            ),
        )

    # 3. Scheduler jobs — each job's last run; any failed job is degraded.
    sh = scheduler_health()
    if not settings.scheduler_enabled:
        add("scheduler", True, critical=False, detail="disabled in config")
    elif not sh:
        add("scheduler", False, critical=False, detail="no job has run yet")
    else:
        failed = [name for name, hb in sh.items() if not hb.get("ok")]
        add(
            "scheduler",
            not failed,
            critical=False,
            detail=(
                f"{len(sh)} jobs; failing: {', '.join(failed)}"
                if failed
                else f"{len(sh)} jobs healthy"
            ),
        )

    # 4. Outbound email — configured is the check we can make without sending.
    ...
    add(
        ...
        detail=(
            ...
            + (" (+ relay fallback)" if settings.smtp_relay_fallback_dsn else "")
            if smtp_ok
            else "no SMTP host — alert email / reset / verification will not send"
        ),
    )

    # 5. AI cascade.
    if settings.llm_provider == "none":
        add("ai_cascade", True, critical=False, detail="disabled (rules-only)")
    else:
        add(
            "ai_cascade",
            True,
            critical=False,
            detail=(
                f"{settings.llm_provider} · {settings.openai_model}"
    ...
    cust = custody_summary()
    add(
        "credential_custody",
        bool(cust.get("ok")),
        critical=True,
        detail=f"{cust.get('mode')} · can_decrypt={cust.get('can_decrypt')}",
    )

    # 7. OAuth providers (informational).
    from envelock.channels.mail.oauth import configured_providers
    ...
    add(
        ...
        True,
        critical=False,
        detail=", ".join(provs) if provs else "none configured (IMAP still works)",
    )

    down = any(c["state"] == "down" for c in checks)
    degraded = any(c["state"] == "degraded" for c in checks)
    overall = "down" if down else ("degraded" if degraded else "operational")
