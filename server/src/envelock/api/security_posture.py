...
def _config_checks() -> list[Check]:
    ...
    # 6. Proxy header trust. Getting this wrong does not look like a security
    # problem to a customer — it looks like sign-in and password reset being
    # broken, because every client shares one rate-limit bucket.
    checks.append(
        Check(
            "forwarded_for",
            "Client-IP identification",
            "pass",
            "X-Forwarded-For is honoured from a local reverse proxy, so each "
            "client gets its own rate-limit bucket. A header arriving directly "
            "from a public address is still ignored, so limits cannot be "
            "bypassed by spoofing it."
            + (
                " ENVELOCK_TRUST_FORWARDED_FOR additionally forces it on for a "
                "remote proxy (a CDN)."
                if settings.trust_forwarded_for
                else ""
            ),
            "If your proxy is REMOTE (Cloudflare, a managed load balancer) its "
            "requests arrive from a public address, so set "
            "ENVELOCK_TRUST_FORWARDED_FOR=true — otherwise every customer on the "
            "platform shares one bucket and sign-in starts failing for everyone."
    ...
    # 7. Outbound transactional mail. Distinct from alert delivery: without it,
    # password reset silently does nothing and a locked-out customer has no way
    # back in unless they hold an authenticator.
    from envelock.notify.mail import is_configured as mail_ok

    checks.append(
        Check(
            "password_reset_email",
            "Password reset by email",
            "pass" if mail_ok() else "fail",
            "Reset links can be delivered."
            if mail_ok()
            else "No SMTP relay is configured, so no reset link can be sent. "
            "Customers with an authenticator can still reset with a code; "
            "everyone else has no way back into their account.",
            "" if mail_ok() else "Set ENVELOCK_SMTP_HOST (and the DKIM key). "
            "`localhost` counts as unconfigured.",
            "high",
        )
    )
...
async def _rls_check(session: AsyncSession) -> Check:
    """Ask Postgres whether tenant isolation is real.

    This used to report `pass` whenever `ENVELOCK_RLS_ENABLED` was true, which
    answers a different question — what someone intended, not what the database
    will do. There are three separate ways for the flag to be on and isolation to
    be absent, and a security page that cannot tell them apart is worse than none:
    it converts "we hope so" into "verified" on the page an operator checks
    before answering a customer's questionnaire.
    """
    from envelock.config import get_settings
    from envelock.db_rls import verify_rls

    settings = get_settings()
    status = await verify_rls(await session.connection(), enabled=settings.rls_enabled)
    ...
    if status.effective:
        ...
        state = "warn"
        remedy = (
            "Run `python -m envelock.security.provision_rls`, point "
            "ENVELOCK_POSTGRES_DSN at the role it creates, then set "
            "ENVELOCK_RLS_ENABLED=true."
        )
    elif status.role_bypasses:
        # The worst case: configured on, believed on, doing nothing.
        state = "fail"
        remedy = (
            "Connect as a role without SUPERUSER/BYPASSRLS. "
            "`python -m envelock.security.provision_rls` creates one."
        )
    else:
        state = "fail"
        remedy = "Restart the API — policies are applied at boot — or check the logs."

    detail = status.summary()
    if not status.enabled_in_config:
        detail += (
            ". Tenant isolation rests entirely on application code remembering a "
            "WHERE tenant_id on every query; one missed filter leaks another "
            "company's data."
        )
    return Check(
        "rls", "Database row-level security", state, detail, remedy, "high"
    )


async def _data_checks(session: AsyncSession) -> list[Check]:
    checks: list[Check] = [await _rls_check(session)]
