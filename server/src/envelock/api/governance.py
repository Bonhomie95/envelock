...
from envelock.auth.deps import AdminUser, CurrentUser, OwnerUser, SystemScoped
...
# ── Outbound SIEM webhooks ───────────────────────────────────────────────────
class WebhookCreateRequest(BaseModel):
    url: str = Field(min_length=8, max_length=2048)
    #: Which events to send. Empty means every event — the least surprising
...
def _endpoint_payload(row, *, secret: str | None = None) -> dict:  # noqa: ANN001
    out = {
        "id": str(row.id),
        "url": row.url,
    ...
    if secret is not None:
        # Returned exactly once, at creation. It is the customer's half of the
        # signature — we keep it to sign with, but never hand it back again.
        out["secret"] = secret
    return out


@router.get("/export/webhooks")
async def list_webhooks(principal: AdminUser, session: Session) -> dict:
    from envelock.models import WebhookEndpoint
    ...
    return {
        "webhooks": [_endpoint_payload(r) for r in rows],
        "events": [e.value for e in ex.WebhookEvent],
        "retry_schedule_seconds": list(ex.RETRY_SCHEDULE),
        "signature_header": "X-Envelock-Signature",
    }


@router.post("/export/webhooks", status_code=201)
async def create_webhook(
    req: WebhookCreateRequest, principal: AdminUser, session: Session
) -> dict:
    """Register a SIEM receiver. Returns the signing secret once.

    The URL is validated here rather than at first delivery, so a typo or an
    internal address is a clear error on the form instead of a delivery that
    fails silently four hours later.
    """
    from envelock.models import WebhookEndpoint
    from envelock.workers.webhook_delivery import (
    ...
    try:
        ...
        # Accepted deliberately. A receiver whose DNS is briefly unavailable, or
        # a record registered moments before its A record propagates, is not a
        # configuration error — the delivery path retries, and the endpoint's
        # last_status shows the customer what is happening.
        pass

    unknown = [e for e in req.events if e not in {w.value for w in ex.WebhookEvent}]
    if unknown:
        raise HTTPException(422, f"unknown event type(s): {', '.join(unknown)}")
    ...
    await alert_svc.record_audit(
        ...
        detail={"url": url, "events": list(req.events)},
    )
    await session.commit()
    return {
        **_endpoint_payload(endpoint, secret=secret),
        "next": (
            "Store this secret — it is shown once. Every delivery carries an "
            "X-Envelock-Signature header computed over the timestamp and body; "
            "verify it before trusting a payload."
        ),
    }


@router.patch("/export/webhooks/{webhook_id}")
async def update_webhook(
    webhook_id: UUID, req: WebhookUpdateRequest, principal: AdminUser, session: Session
) -> dict:
    from envelock.models import WebhookEndpoint
    ...
    if req.events is not None:
        ...
        if unknown:
            raise HTTPException(422, f"unknown event type(s): {', '.join(unknown)}")
        endpoint.events = list(req.events)
    await session.commit()
    return _endpoint_payload(endpoint)


@router.delete("/export/webhooks/{webhook_id}", status_code=204)
async def delete_webhook(
    webhook_id: UUID, principal: AdminUser, session: Session
) -> Response:
    from envelock.models import WebhookEndpoint
...
# SystemScoped: authenticated by an export token rather than a user session,
# and the tenant is resolved FROM that token. Without this the feed a
# customer points Splunk at would authenticate correctly and then return
# an empty alert list forever.
@router.get("/export/api/alerts", dependencies=[SystemScoped])
