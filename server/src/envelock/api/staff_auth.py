...
async def staff_refresh(req: StaffTokenRequest, session: Session) -> dict:
    ...
    # Atomic first-use claim — see the customer-side refresh for why the
    # read-then-write pair raced under concurrent replay.
    if not await active_revocations().aconsume_jti(
        claims.jti, expires_at=float(claims.exp)
    ):
        await active_revocations().arevoke_user(
            str(claims.sub),
            until=datetime.now(UTC).timestamp() + REFRESH_TTL.total_seconds(),
        )
        raise HTTPException(
            status.HTTP_401_UNAUTHORIZED,
            "For your security, you've been signed out of all devices.",
...
async def current_operator(
    ...
    * a customer session whose email is on `ENVELOCK_SUPERADMIN_EMAILS` — the
    ...
    # An EXPIRED/invalid token is 401 ("re-authenticate"), distinct from 404
    # ("this surface isn't yours"). The client refreshes on 401 only; when this
    # returned 404 for an expired access token, the 14-day refresh token was
    # never exchanged and the operator was bounced to "not an administrator"
    # after 15 minutes. Absent/garbage authorization still hides the surface.
    ...
    # 1. A staff token. A token that IS a staff_access token but has expired must
    # tell the client to refresh (401), not read as "not an admin" (404).
    staff_token_expired = False
    try:
        claims = decode_token(raw, expect=ACCESS_TYPE)
    except TokenError as exc:
        claims = None
        staff_token_expired = "expired" in str(exc).lower()
    if staff_token_expired:
        raise expired
    if claims is not None:
        # Logout revokes the operator's token family; without this read-side
        # check, a stolen staff access token survived "sign out" for its full
        # TTL — against the cross-tenant console. Same fix as the customer-side
        # `current_principal`.
        if await active_revocations().ais_revoked(
            claims.jti, str(claims.sub), issued_at=claims.iat
        ):
    ...
    if await active_revocations().ais_revoked(
        user_claims.jti, str(user_claims.sub), issued_at=user_claims.iat
    ):
        raise not_found
    allow = get_settings().superadmin_email_set
    user = await session.get(User, user_claims.sub)
    if user is None or not allow or user.email.lower() not in allow or user.status != "active":
        raise not_found
    # Same rule as `superadmin_actor`: break-glass is every permission over every
    # tenant, so it does not ride on an account that skipped two-factor. Staff
    # accounts proper already cannot reach the console without enrolling.
    if not user.mfa_enabled:
        raise HTTPException(
            status.HTTP_403_FORBIDDEN,
            "break-glass access requires two-factor authentication on your "
            "Envelock account. Enrol an authenticator, then sign in again.",
        )
...
# Moved to auth/staff.py so non-API modules (security/bootstrap_staff) never
# import a router module for it. Re-exported here for existing importers.
from envelock.auth.staff import new_temporary_password  # noqa: E402, F401
