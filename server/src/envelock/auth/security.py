...
class TokenClaims:
    ...
    exp: int
    jti: str
    #: Issue time, with sub-second precision. Without it a user-level revocation
    #: could only be a blanket ban on the subject, which meant signing out locked
    #: the account out of its own refresh flow — see
    #: `security/limits.TokenRevocations`.
    #:
    #: Fractional deliberately: whole seconds would make a token minted in the
    #: same second as a sign-out compare as "issued before the cutoff", so
    #: signing straight back in would fail for the remainder of that second.
    #: `None` for a token minted before this claim existed; the revocation check
...
def decode_token(token: str, *, expect: str | None = None) -> TokenClaims:
    ...
    try:
        ...
        claims = TokenClaims(
            ...
            exp=int(data["exp"]),
            jti=data["jti"],
            # `.get`, not `[...]`: a token issued before this claim existed must
            # still decode, so a deploy does not sign out every open session at
            # once. The revocation check treats a missing value as revoked.
            iat=float(data["iat"]) if data.get("iat") is not None else None,
        )
