...
async def current_principal(
    ...
    # Signing out has to end the session that is signed in. `/auth/logout` and the
    # password-change paths revoke the user's whole token family, but nothing
    # checked that here — so only refresh tokens actually died, and the access
    # token in the attacker's hands stayed valid for its full 15-minute TTL after
    # the victim pressed "sign out of all devices". The revocation store already
    # existed; this is the read side of it.
...
async def optional_principal(
    ...
    try:
        ...
        return None
    # A revoked session must read as anonymous here too, or signing out would
    # still leave the caller with the signed-in view of the public endpoints.
    if await active_revocations().ais_revoked(
        claims.jti, str(claims.sub), issued_at=claims.iat
    ):
        return None
...
async def system_scoped():  # noqa: ANN201
    """Router-level dependency: this router may cross tenant boundaries.

    Two kinds of route legitimately need it, and only two:

    * **Pre-authentication.** Sign-in has to find a user by email before any
      tenant is known — that is what signing in *is*. Under RLS with no tenant
      bound, `select(User).where(email == ...)` matches nothing, so login,
      registration and password reset would all fail closed. Correct behaviour
      for a data route; fatal for these.
    * **Deliberately cross-tenant.** The operator console and the platform
      workers exist to see every tenant.

    Everything that serves a customer their own data — alerts, mailboxes,
    messages, counterparties, governance — is deliberately NOT given this, so
    the database still refuses a query there that forgets its tenant filter.
    That is where the mail lives and where the backstop earns its keep.
    """
    from envelock.db_rls import system_scope

    with system_scope("router declared pre-tenant or cross-tenant"):
        yield


#: Attach to a router with `dependencies=[SystemScoped]`.
SystemScoped = Depends(system_scoped)
...
async def superadmin_actor(
    ...
    # Break-glass hands out every permission over every tenant, and customer MFA
    # is deliberately skippable (a choice that is right for customers and wrong
    # here). Without this, a super-admin who clicked "skip for now" was one
    # password away from every mailbox on the platform — and the admin API is
    # served from the public API host.
    #
    # A clear 403 rather than the 404 above: the 404 exists so a stranger cannot
    # learn this surface exists, and the caller here has already proven they hold
    # a valid session for an allowlisted address. Telling *them* what to do is
    # not an oracle; leaving them at a 404 just looks like we are broken.
    if not user.mfa_enabled:
        raise HTTPException(
            status.HTTP_403_FORBIDDEN,
            "platform administration requires two-factor authentication on your "
            "account. Enrol an authenticator from your profile, then sign in again.",
        )
    return Actor(
        user_id=user.id,
        tenant_id=user.tenant_id,
