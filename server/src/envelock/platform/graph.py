...
class CounterpartyGraph:
    ...
    def public_stats(self) -> dict[str, int]:
        """Counts only — safe to show an anonymous visitor.

        The shared graph is the one thing a competitor cannot copy on their first
        day, and until now nothing outside the product said it existed. What can
        be published is the shape of the network effect: how many domains the
        network has judged, and how many have enough independent confirmations to
        act on. Never a domain name (that is a target list and a defamation risk
        on a false positive), never a count of tenants, never who reported what.
        """
        fraudulent = [
            e for e in self._entries.values() if e.verdict is Verdict.FRAUDULENT
        ]
        return {
            "domains_judged": len(self._entries),
            "domains_confirmed_fraudulent": len(fraudulent),
            "actionable": sum(1 for e in fraudulent if e.actionable),
            "confirmations": sum(e.confirmations for e in fraudulent),
        }
...
def plan_backfill(
    *,
    ...
    full_days: int | None = None,
) -> BackfillPlan:
    """Capped during trial: 90 days of analysis before the customer has paid is
    the expensive part of a trial (PRD §12.7). Windows come from settings
    (ENVELOCK_TRIAL_BACKFILL_DAYS / ENVELOCK_BACKFILL_DAYS) — both existed and
    were read by nothing, the caps were hardcoded."""
    from envelock.config import get_settings

    s = get_settings()
    trial_days = s.trial_backfill_days if trial_days is None else trial_days
    full_days = s.backfill_days if full_days is None else full_days
