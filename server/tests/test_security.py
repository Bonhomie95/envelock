...
def test_forwarded_for_from_the_public_internet_is_ignored() -> None:
    """A spoofable X-Forwarded-For must not split rate-limit buckets when the
    request came straight from the internet — otherwise anyone bypasses
    throttling by rotating the header."""
    ...
    def _req(xff: str):
        class _R:
            ...
            class client:  # noqa: N801
                # A routable address: this request was NOT forwarded by us.
                host = "8.8.8.8"
    ...
    assert a == b == "ip:8.8.8.8"  # both bucketed by the real peer, XFF ignored


def test_forwarded_for_from_our_own_proxy_identifies_the_real_client() -> None:
    """The opposite failure is worse and was the live one: with nginx on the same
    box proxying to 127.0.0.1, using the peer address puts EVERY customer in one
    bucket, so "10 sign-ins per 5 minutes" becomes ten for the whole platform.
    A request whose peer is local was forwarded by us, so its header is ours."""
    from starlette.datastructures import Headers

    from envelock.config import get_settings
    from envelock.security.middleware import client_identity

    def _req(peer: str, xff: str):
        class _R:
            headers = Headers({"x-forwarded-for": xff})
    ...
    assert client_identity(_req("127.0.0.1", "1.2.3.4, 127.0.0.1")) == "ip:1.2.3.4"
    assert client_identity(_req("10.0.0.1", "5.6.7.8")) == "ip:5.6.7.8"
