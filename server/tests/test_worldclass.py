...
def test_reporting_a_lookalike_persists_a_verdict_row(client: TestClient) -> None:
    ...
    h, _ = _admin(client)
    # Reports are only accepted for lookalikes WE surfaced to this tenant — an
    # arbitrary path string would be a cross-tenant blocklist primitive. Seed
    # the watch-list row the CT watcher would normally have written.
    import asyncio as _a

    from conftest import platform_sessionmaker as _sm
    ...
    async def _seed() -> None:
        ...
        async with _sm()() as s:
            s.add(
                LookalikeDomain(
                    tenant_id=_UUID(me["tenant_id"]),
                    protected_domain="acme.com",
                    candidate_domain="evil-lookalike.com",
                    technique="cousin",
                    similarity=0.9,
                    first_seen_source="ct",
                )
            )
            await s.commit()
