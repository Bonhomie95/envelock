...
# ── Break-glass requires two-factor ──────────────────────────────────────────
def test_break_glass_refuses_an_account_without_two_factor(
    client: TestClient, monkeypatch
) -> None:
    """Break-glass is every permission over every tenant, and customer MFA is
    deliberately skippable — so without this check a super-admin who clicked
    "skip for now" was one password away from every mailbox on the platform,
    over the public API host.
    """
    from envelock.config import get_settings

    email = "breakglass-nomfa@envelock.org"
    pw = "a-long-enough-passphrase"
    client.post(
        "/api/v1/auth/register",
        json={"email": email, "password": pw, "tenant_name": "Envelock"},
    )
    login = client.post("/api/v1/auth/login", json={"email": email, "password": pw}).json()
    # Skip two-factor, which the product allows.
    skipped = client.post("/api/v1/auth/mfa/skip", json={"token": login["mfa_token"]}).json()
    headers = {"Authorization": f"Bearer {skipped['access_token']}"}

    monkeypatch.setenv("ENVELOCK_SUPERADMIN_EMAILS", email)
    get_settings.cache_clear()
    try:
        resp = client.get("/api/v1/admin/overview", headers=headers)
        assert resp.status_code == 403, resp.text
        assert "two-factor" in resp.json()["detail"]
    finally:
        get_settings.cache_clear()
