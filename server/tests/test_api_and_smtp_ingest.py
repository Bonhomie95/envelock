...
async def test_smtp_handler_accepts_known_and_rejects_unknown_recipients():
    ...
    good = await handler.handle_RCPT(None, None, env, "t-abc12345@in.envelock.org", [])
    assert good == "250 OK"
    assert env.rcpt_tos == ["t-abc12345@in.envelock.org"]

    bad = await handler.handle_RCPT(None, None, env, "nobody@in.envelock.org", [])
...
async def test_smtp_handler_runs_data_through_ingest():
    ...
    handler = ForwardingSMTPHandler(ingest=ingest)
    env = _Envelope()
    env.rcpt_tos = ["t-abc12345@in.envelock.org"]
    env.content = _raw()

    reply = await handler.handle_DATA(None, None, env)
    assert reply.startswith("250")
    assert ingest.datas and ingest.datas[0][0] == "t-abc12345@in.envelock.org"
