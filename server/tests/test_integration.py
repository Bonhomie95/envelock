...
async def test_whole_journey_signup_to_acknowledged_alert(
    ...
    assert boot["ingest_address"].endswith("in.envelock.org")
    ...
    # A validated JSON body, not a bare query parameter. This number is shown to
    # someone about to stop a payment, so an empty or malformed one is worse than
    # none — it reads as a verified callback and is not one. Asserting the status
    # matters here: the previous form silently did nothing, and the failure only
    # surfaced further down as a missing callback on the alert.
    phone_set = client.post(
        "/api/v1/counterparties/gemini.com/phone",
        json={"phone": "+1 803 000 0000"},
        headers=h,
    )
    assert phone_set.status_code == 200, phone_set.text
