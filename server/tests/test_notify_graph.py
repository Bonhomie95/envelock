...
async def _tenant_with_admin(session, *, email: str, phone: str | None = None):
    ...
    session.add(tenant)
    # Feeding the cross-tenant graph now requires a DNS-verified domain — the
    # anti-poisoning gate (two throwaway tenants must not blocklist the world).
    session.add(
        Domain(
            id=uuid4(),
            tenant_id=tenant.id,
            name=f"verified-{uuid4().hex[:8]}.com",
            registrable_domain=f"verified-{uuid4().hex[:8]}.com",
