...
def _configure(**env: str | None) -> None:
    for key, value in env.items():
        if value is None:
            # An EMPTY env var, not a popped one: popping falls back to the
            # developer's own .env file, whose real x25519 keys then collide
            # with the test-generated pair. Env vars outrank the file, so ""
            # is how a test says "this key is genuinely not set".
            os.environ[key] = ""
        else:
            os.environ[key] = value
    get_settings.cache_clear()
...
# These tests are about KEY CUSTODY, so satisfy the other production boot gates
# up front. Their own refusals are covered in test_production_boot_gates.py —
# leaving them unset here would make every custody assertion pass or fail for the
# wrong reason.
os.environ["ENVELOCK_RLS_ENABLED"] = "true"
os.environ["ENVELOCK_REQUIRE_EMAIL_VERIFICATION"] = "true"
os.environ["ENVELOCK_SMTP_HOST"] = "smtp.example.com"
