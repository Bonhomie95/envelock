...
class TinyImapServer(threading.Thread):
    ...
    def __init__(self, *, password: str) -> None:
        ...
        #: UIDs whose \Seen flag a client set by fetching the body without PEEK
        #: from a read-write SELECT — exactly what a real server does (RFC 3501
        #: §6.4.5). Envelock must never appear here: marking a customer's new
        #: mail as read is the opposite of protecting it.
        self.marked_seen: set[int] = set()
        #: The raw data items of every UID FETCH, so a test can assert on them.
            ...
            while True:
                ...
                while b"\r\n" in buffer:
                    ...
                    if command == "CAPABILITY":
                        ...
                        # A body fetch without PEEK on a read-write folder sets
                        # \\Seen — the server-side behaviour under test.
                        if not peek and read_write and (
                            "RFC822" in items or "BODY[]" in items
                        ):
                            self.marked_seen.add(1001)
                        # Servers answer BODY.PEEK[] under the name BODY[].
                        name = "BODY[]" if peek or "BODY[]" in items else "RFC822"
                        send(f"* 1 FETCH (UID 1001 {name} {{{len(MESSAGE)}}}")
...
def imap_server() -> TinyImapServer:
    ...
    # deployment turns this on, and so does this test. The same deployment is the
    # only one allowed to speak unencrypted IMAP (the server here is plaintext on
    # a LAN socket), so it opts into that explicitly too.
    os.environ["ENVELOCK_IMAP_ALLOW_PRIVATE_HOSTS"] = "true"
    os.environ["ENVELOCK_IMAP_ALLOW_PLAINTEXT"] = "true"
    ...
    os.environ["ENVELOCK_IMAP_ALLOW_PLAINTEXT"] = "false"
    get_settings.cache_clear()
...
def test_polling_never_marks_the_customers_mail_as_read(
    imap_server: TinyImapServer,
) -> None:
    """The regression this server's \\Seen bookkeeping exists to catch.

    The live poll opens the inbox read-write — it has to, to quarantine in the
    same session — and fetched `RFC822`, which on a read-write folder sets
    \\Seen (RFC 3501 §6.4.5). Every new message in every connected mailbox was
    marked read the instant Envelock looked at it, so customers silently stopped
    seeing their new mail. It also made silent-access detection impossible,
    because the answer to "who read this?" was always us.
    """
    from envelock.channels.mail import imap_sync
    ...
    assert result.ok, result.error
    assert result.messages, "the message still has to arrive"
    assert imap_server.marked_seen == set(), (
        f"Envelock set \\Seen on {sorted(imap_server.marked_seen)} — "
        f"fetch items were {imap_server.fetch_items}"
    )
    assert all("BODY.PEEK[]" in items for items in imap_server.fetch_items)
