...
def test_confirm_opens_the_gate_and_starts_the_trial(
    ...
    try:
        ...
        # A Stripe PaymentMethod is mintable client-side with the publishable
        # key and any Luhn-valid PAN — verifying the OBJECT is not a payment.
        # The gate therefore does NOT open here; hosted Checkout's paid webhook
        # is the single source of entitlement, and the response points there.
        assert body["gate_passed"] is False
        assert body["next"] == "checkout"
        assert body["trial_allowed"] is True  # informational eligibility
        ...
        # The signup trial is untouched; the payment flag is not set by /confirm.
...
def test_colleagues_share_one_tenant_and_one_trial(
    ...
    try:
        ...
        # The trial began at signup; card verification alone no longer opens the
        # gate (that takes the paid Checkout webhook).
