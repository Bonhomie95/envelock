...
def test_an_operator_cannot_skip_two_factor(client: TestClient) -> None:
    ...
    _seed("ada@envelock.org", Department.LEADERSHIP)
    login = client.post(
        "/api/v1/admin/auth/login",
        json={"email": "ada@envelock.org", "password": REAL_PW},
    )
...
def test_a_new_operator_must_replace_the_temporary_password_first(
    ...
    headers = _sign_in(client, "new@envelock.org", TEMP_PW)
    ...
    # the console answers.
    headers = _sign_in(client, "new@envelock.org", REAL_PW)
...
def test_support_can_approve_a_user_but_not_suspend_a_tenant(client: TestClient) -> None:
    _seed("sam@envelock.org", Department.SUPPORT)
    headers = _sign_in(client, "sam@envelock.org")
...
def test_billing_cannot_read_the_audit_trail_or_customer_users(
    ...
    _seed("bill@envelock.org", Department.BILLING)
    headers = _sign_in(client, "bill@envelock.org")
    assert client.get("/api/v1/admin/tenants", headers=headers).status_code == 200
...
def test_compliance_reads_everything_and_changes_nothing(client: TestClient) -> None:
    _seed("cara@envelock.org", Department.COMPLIANCE)
    headers = _sign_in(client, "cara@envelock.org")
...
def test_creating_a_colleague_returns_a_one_time_password(client: TestClient) -> None:
    _seed("lead@envelock.org", Department.LEADERSHIP)
    headers = _sign_in(client, "lead@envelock.org")

    created = client.post(
        "/api/v1/admin/staff",
        json={
            "email": "Nina@Envelock.ORG",
            "name": "Nina",
            "department": "support",
        },
        headers=headers,
    ...
    body = created.json()
    assert body["email"] == "nina@envelock.org"
    ...
    nina = next(s for s in listed["staff"] if s["email"] == "nina@envelock.org")
    ...
    assert (
        client.post(
            "/api/v1/admin/auth/login",
            json={"email": "nina@envelock.org", "password": body["temporary_password"]},
        ).status_code
        == 200
    )
...
def test_nobody_can_grant_a_permission_they_do_not_hold(client: TestClient) -> None:
    ...
    _seed("sec@envelock.org", Department.SECURITY)
    headers = _sign_in(client, "sec@envelock.org")
    ...
    assert (
        client.post(
            "/api/v1/admin/staff",
            json={"email": "mole2@envelock.org", "department": "leadership"},
            headers=headers,
        ).status_code
        == 403
    )
...
def test_an_operator_cannot_promote_themselves(client: TestClient) -> None:
    staff_id = _seed("sec2@envelock.org", Department.SECURITY)
    headers = _sign_in(client, "sec2@envelock.org")
...
def test_suspending_an_operator_takes_effect_immediately(client: TestClient) -> None:
    _seed("lead2@envelock.org", Department.LEADERSHIP)
    victim_id = _seed("temp@envelock.org", Department.SUPPORT)
    lead_headers = _sign_in(client, "lead2@envelock.org")
    victim_headers = _sign_in(client, "temp@envelock.org")
...
def test_the_last_staff_manager_cannot_be_suspended(client: TestClient) -> None:
    ...
    only_id = _seed("solo@envelock.org", Department.LEADERSHIP)
    headers = _sign_in(client, "solo@envelock.org")
    ...
    other_id = _seed("solo2@envelock.org", Department.SECURITY)
    other_headers = _sign_in(client, "solo2@envelock.org")
...
def test_a_password_reset_also_clears_the_authenticator(client: TestClient) -> None:
    ...
    _seed("lead3@envelock.org", Department.LEADERSHIP)
    target_id = _seed("lost@envelock.org", Department.SUPPORT)
    headers = _sign_in(client, "lead3@envelock.org")
...
def test_every_staff_action_is_written_to_the_staff_audit_log(client: TestClient) -> None:
    _seed("lead4@envelock.org", Department.LEADERSHIP)
    headers = _sign_in(client, "lead4@envelock.org")
    client.post(
        "/api/v1/admin/staff",
        json={"email": "audited@envelock.org", "department": "billing"},
        headers=headers,
    )
    events = client.get("/api/v1/admin/staff/audit", headers=headers).json()["events"]
    assert any(
        e["action"] == "staff.created" and e["actor"] == "lead4@envelock.org"
        for e in events
    )
...
def test_the_role_catalogue_says_what_each_department_grants(client: TestClient) -> None:
    ...
    _seed("lead5@envelock.org", Department.LEADERSHIP)
    headers = _sign_in(client, "lead5@envelock.org")
...
def test_the_security_page_reports_the_deployment_honestly(client: TestClient) -> None:
    _seed("sec3@envelock.org", Department.SECURITY)
    headers = _sign_in(client, "sec3@envelock.org")
...
def test_system_status_rolls_up_every_component(client: TestClient) -> None:
    """The ops status page: one honest health view, bounded checks, an overall
    verdict that is the worst state present."""
    _seed("ops@envelock.org", Department.LEADERSHIP)
    headers = _sign_in(client, "ops@envelock.org")
    body = client.get("/api/v1/admin/status/system", headers=headers).json()

    assert body["overall"] in ("operational", "degraded", "down")
    components = {c["component"] for c in body["checks"]}
    # Every moving part is represented.
    assert {"database", "redis", "imap_poll_worker", "scheduler",
            "outbound_email", "ai_cascade", "credential_custody"} <= components
    # The DB is reachable in the test env, so it must read up.
    db = next(c for c in body["checks"] if c["component"] == "database")
    assert db["state"] == "up" and db["critical"] is True
    # Overall is the worst state present — never rosier than its checks.
    states = {c["state"] for c in body["checks"]}
    if "down" in states:
